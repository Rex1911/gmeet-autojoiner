...
class Gmeet {
	...
	join = async (url, options = {}) => {
		...
		do {
			try {
				this.browser = await puppeteer.launch({headless: headless, args: ['--use-fake-ui-for-media-stream']});
				page = await this.browser.newPage()
				await page.goto(url)
				await helper.waitForXSec(10);
				if(verbose) console.log("[VERBOSE] Inputting username")
				await page.type("input#identifierId", "arusociotown", {delay: 100})
				...
				await page.type("input.whsOnd.zHQkBf", "Harrypoter@123", {delay: 100})
				await page.click("div.ZFr60d.CeoRYc")
				if(verbose) console.log("[VERBOSE] Clicked the next button")
				await helper.waitForXSec(5)
				await page.goto(url)
				...
				console.log("[ERROR]Something went wrong, trying again")
				console.log(err)
				await page.close()
				await this.browser.close()
				failed = true;
